...
def run_no_gui(device_count=10, duration=0, interval=1.0, api_url=None, api_token=None, login_url=None, verify_ssl=True, use_mq=True, encrypt=False, local_store=True):
    import time
    ...
    try:
        with ThreadPoolExecutor(max_workers=min(device_count, 2000)) as executor:
            while True:
...
def main():
    try:
        ...
        parser.add_argument("--no-local-store", action="store_true", help="Disable simulator local JSON writes")
        ...
        if args.no_gui:
            run_no_gui(
                ...
                local_store=not args.no_local_store,
            )
            return
